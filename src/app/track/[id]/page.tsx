...
// The 8 Logistics Steps as defined in your proposal
...
export default async function TrackingPage({ params }: { params: { id: string } }) {
    ...
    try {
        ...
        // Fetch the specific request. lean() converts it to a plain JS object.
        requestData = await Request.findById(params.id).lean();
    } catch (error) {
        // If the ID is an invalid format, it will throw an error
        console.error("Invalid Request ID");
    }

    // If no request is found, show a 404
    if (!requestData) {
        notFound();
    }
    ...
    // Fallback to step 1 if status isn't perfectly mapped yet
            ...
            <div className="relative z-10 max-w-4xl mx-auto px-6 pt-32">
                ...
                <div className="text-center mb-16 border-b border-white/10 pb-12">
                    ...
                    <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-white mb-4">
                        {requestData.make} {requestData.model}
                    </h1>
                            ...
                            return (
                                <div key={step.id} className="flex items-start gap-8">
                                    ...
                                    <div className={`pt-3 transition-all duration-500 ${isFuture ? "opacity-40" : "opacity-100"}`}>
                                        ...
                                        {/* Mock Tracking Details if Shipped is Active */}
                                        ...
                                        {/* Mock Garage Notes if Processing is Active */}
