...
import {
    TrendingUp, DollarSign, Target, Award, ArrowUpRight,
    Zap, Clock, CheckCircle2, Briefcase, AlertCircle, RefreshCw, BarChart3, PieChart, Loader2
} from "lucide-react";
...
export default function OperationsOverviewDashboard() {
    ...
    const pipelineVelocity = [
        { stage: "New", count: dashboardData?.pipelineStages?.New || 0, color: "bg-zinc-900" },
        { stage: "Vehicle Selection", count: dashboardData?.pipelineStages?.["Vehicle Selection"] || 0, color: "bg-zinc-700" },
        { stage: "Shipped", count: dashboardData?.pipelineStages?.["Shipped"] || 0, color: "bg-zinc-500" },
        { stage: "Cleared Customs", count: dashboardData?.pipelineStages?.["Cleared Customs"] || 0, color: "bg-zinc-300" },
    ];
        ...
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto min-h-screen">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-black/5">
                <div>
                    ...
                    <p className="text-zinc-500 mt-1">Cross-team performance statistics, conversion ratios, and pipeline health.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white border border-black/5 rounded-[2rem] p-6 shadow-sm">
                    <p className="text-xs font-bold uppercase text-zinc-400 mb-1">Gross Managed Pipeline</p>
                    <h3 className="text-3xl font-bold font-mono">${(dashboardData?.grossPipeline || 0).toLocaleString()}</h3>
                </div>
                <div className="bg-white border border-black/5 rounded-[2rem] p-6 shadow-sm">
                    <p className="text-xs font-bold uppercase text-zinc-400 mb-1">Total Inquiries</p>
                    <h3 className="text-3xl font-bold font-mono">{dashboardData?.totalRequests || 0}</h3>
                </div>
                            ...
                            {pipelineVelocity.map((bar, i) => (
                                <div key={i} className="space-y-1">
                                    ...
                                    <div className="w-full h-3 bg-zinc-100 rounded-full overflow-hidden">
                                        <div className={`h-full ${bar.color}`} style={{ width: `${Math.max(10, (bar.count / (dashboardData?.totalRequests || 1)) * 100)}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                            ...
                            {dashboardData?.activityStream?.map((log: any) => (
                                <div key={log.id} className="relative pl-8 text-sm">
                                    ...
                                    <div className="flex justify-between mb-1">
                                        ...
                                        <span className="text-[10px] text-zinc-400">{new Date(log.time).toLocaleTimeString()}</span>
                                    </div>
                                    ...
                                    {log.comment && (
                                        <div className="mt-2 p-3 bg-zinc-50 rounded-xl text-xs text-zinc-500 italic">"{log.comment}"</div>
                                    )}
                                </div>
                            ))}
                        </div>
