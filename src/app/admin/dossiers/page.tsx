...
export default function InventoryPage() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Are you sure? This will permanently remove this template.")) return;

        startTransition(async () => {
            const res = await deleteSpecDossier(id);
            if (res.success) {
    ...
    const filteredDossiers = dossiers.filter(d => {
        ...
        return (
            ...
            d.countryOfOrigin?.toLowerCase().includes(query)
        );
    });

    // 4. Calculate Stats (checks for both new and legacy statuses)
    const activeCount = dossiers.filter(d => d.status === "Active" || d.status === "Published").length;
    const draftCount = dossiers.filter(d => d.status === "Draft" || !d.status).length;

    // Helper: Safe Image Resolver
    ...
    const getStatusColor = (status: string) => {
        // Handles new "Active" and legacy "Published"
        if (status === 'Active' || status === 'Published') {
            return 'bg-emerald-100 text-emerald-800 border-emerald-200';
        }
        // Handles new "Archived" and legacy "Sold"
    ...
    return (
        <div className="p-6 lg:p-10 space-y-8 max-w-[1600px] mx-auto pb-24">
            ...
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-zinc-900">Spec Templates</h1>
                    <p className="text-zinc-500 mt-1">Master Vehicle Blueprints | Providence Auto</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                <div className="lg:col-span-8 relative">
                    ...
                    <Input
                        placeholder="Search by Make, Model, Year, or Origin..."
                ...
                <div className="lg:col-span-4 flex gap-4">
                    <div className="flex-1 bg-zinc-900 rounded-[1.5rem] p-4 text-white shadow-sm">
                        <p className="text-[9px] uppercase font-bold text-zinc-400 tracking-[0.2em]">Active Specs</p>
                        <p className="text-2xl font-black italic text-white">{activeCount}</p>
                    </div>
                    <div className="flex-1 bg-white border border-black/5 rounded-[1.5rem] p-4 shadow-sm">
                        <p className="text-[9px] uppercase font-bold text-zinc-400 tracking-[0.2em]">Drafts</p>
            ...
            <div className="bg-white border border-black/5 rounded-[2.5rem] overflow-hidden shadow-sm min-h-[400px]">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-[400px] text-zinc-400">
                        ...
                        <p className="font-medium">No templates found matching your search.</p>
                    </div>
                ) : (
                    <Table>
                        <TableHeader className="bg-zinc-50/50 border-b border-black/5">
                            <TableRow className="border-black/5 hover:bg-transparent">
                                <TableHead className="py-6 pl-10 text-[11px] font-black uppercase text-zinc-400 tracking-widest">Template Overview</TableHead>
                                <TableHead className="text-[11px] font-black uppercase text-zinc-400 tracking-widest">Powertrain & Specs</TableHead>
                                <TableHead className="text-[11px] font-black uppercase text-zinc-400 tracking-widest">Status</TableHead>
                                    ...
                                    <TableRow key={d._id} className="border-black/5 group hover:bg-zinc-50/50 transition-colors">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-medium text-sm text-zinc-700">
                                                        {d.engineConfig || "Engine N/A"} {d.displacement ? `(${d.displacement})` : ""}
                                                    </span>
                                                </div>
                                                <div className="flex gap-2">
                                                    <Badge variant="outline" className="text-[10px] bg-white border-black/10 text-zinc-500 px-2 py-0 gap-1">
                                                        <Globe size={10} /> {d.countryOfOrigin || "Global"}
                                                    </Badge>
                                                    <Badge variant="outline" className="text-[10px] bg-white border-black/10 text-zinc-500 px-2 py-0 gap-1">
                                                        <Tag size={10} /> {d.features?.length || 0} Features
                                                    </Badge>
                                                </div>
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col items-start gap-1.5">
                                                <Badge className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${getStatusColor(d.status)}`}>
                                                    {d.status || "Draft"}
                                                </Badge>
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell className="pr-10 text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 shadow-2xl border-black/5 bg-white">
                                                    <DropdownMenuLabel className="text-[10px] font-black uppercase text-zinc-400 px-3 py-2">Template Control</DropdownMenuLabel>
                                                    <DropdownMenuSeparator className="bg-black/5" />

                                                    {/* Updated Edit Route */}
                                                    <Link href={`/admin/specs?editId=${d._id}`}>
                                                        <DropdownMenuItem className="gap-3 rounded-xl cursor-pointer py-3 font-medium hover:bg-zinc-50">
                                                            <Edit3 size={16} className="text-zinc-500" /> Edit Blueprint
                                                        </DropdownMenuItem>
                                                    </Link>
