...
export default function InventoryPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure? This will permanently remove this template."))
      return;
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-zinc-500 mt-1">
            Master Vehicle Blueprints | Providence Auto
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
        ...
        <div className="lg:col-span-4 flex gap-4">
          <div className="flex-1 bg-zinc-900 rounded-[1.5rem] p-4 text-white shadow-sm">
            <p className="text-[9px] uppercase font-bold text-zinc-400 tracking-[0.2em]">
              Active Specs
            </p>
          ...
          <div className="flex-1 bg-white border border-black/5 rounded-[1.5rem] p-4 shadow-sm">
            ...
            <p className="text-2xl font-black italic text-zinc-900">
              {draftCount}
            </p>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className="bg-zinc-50/50 border-b border-black/5">
              <TableRow className="border-black/5 hover:bg-transparent">
                <TableHead className="py-6 pl-10 text-[11px] font-black uppercase text-zinc-400 tracking-widest">
                  Template Overview
                </TableHead>
                <TableHead className="text-[11px] font-black uppercase text-zinc-400 tracking-widest">
                  Powertrain & Specs
                </TableHead>
                <TableHead className="text-[11px] font-black uppercase text-zinc-400 tracking-widest">
                  Status
                </TableHead>
                <TableHead className="text-[11px] font-black uppercase text-zinc-400 tracking-widest text-right pr-10">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredDossiers.map((d) => {
                ...
                return (
                  <TableRow
                    key={d._id}
                    className="border-black/5 group hover:bg-zinc-50/50 transition-colors"
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2">
                          <Badge
                            variant="outline"
                            className="text-[10px] bg-white border-black/10 text-zinc-500 px-2 py-0 gap-1"
                          >
                    ...
                    <TableCell>
                      <div className="flex flex-col items-start gap-1.5">
                        <Badge
                          className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${getStatusColor(d.status)}`}
                        >
                          {d.status || "Draft"}
                        </Badge>
                      </div>
                    ...
                    <TableCell className="pr-10 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            className="h-10 w-10 p-0 rounded-xl hover:bg-zinc-900 hover:text-white transition-colors"
                          >
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="w-56 rounded-2xl p-2 shadow-2xl border-black/5 bg-white"
                        >
                          <DropdownMenuLabel className="text-[10px] font-black uppercase text-zinc-400 px-3 py-2">
                            Template Control
                          </DropdownMenuLabel>
                          ...
                          <Link href={`/admin/specs?editId=${d._id}`}>
                            <DropdownMenuItem className="gap-3 rounded-xl cursor-pointer py-3 font-medium hover:bg-zinc-50">
                              <Edit3 size={16} className="text-zinc-500" /> Edit
                              Blueprint
                            </DropdownMenuItem>
                          </Link>
                          ...
                          <DropdownMenuItem
                            ...
                            className="gap-3 rounded-xl cursor-pointer py-3 font-medium hover:bg-zinc-50"
                          >
                            <Copy size={16} className="text-zinc-500" /> Copy
                            URL
                          </DropdownMenuItem>
