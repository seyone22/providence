...
export default function RequestTableClient({
  ...
  const getPipelineBadge = (status: string) => {
    const current =
      status === "New" ? "Request Initiated" : status || "Action required";
    const isMiddle = [
      "Deposit Collected",
      "Vehicle Purchased",
      "Preparation",
    ...
    if (isDone)
      return (
        <Badge className="bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100 rounded-full px-3">
          {current}
        </Badge>
      );
    if (isShipping)
      return (
        <Badge className="bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100 rounded-full px-3">
          {current}
        </Badge>
      );
    if (isMiddle)
      return (
        <Badge className="bg-purple-50 text-purple-600 border-purple-200 hover:bg-purple-100 rounded-full px-3">
          {current}
        </Badge>
      );

    return (
      <Badge className="bg-zinc-100 text-zinc-600 border-zinc-200 hover:bg-zinc-200 rounded-full px-3">
        {current}
      </Badge>
    );
  };
  ...
  const getSalesStatusBadge = (status: string) => {
    ...
    if (
      ["Action required", "Lead Lost", "Lead Closed", "Not Qualified"].includes(
        current,
      )
    ) {
      return (
        <Badge className="bg-red-50 text-red-600 border-red-200 hover:bg-red-100 rounded-full px-3">
          {current}
        </Badge>
      );
    }
    ...
    if (["No Response", "Stopped Responding"].includes(current)) {
      return (
        <Badge className="bg-yellow-50 text-yellow-600 border-yellow-200 hover:bg-yellow-100 rounded-full px-3">
          {current}
        </Badge>
      );
    }
    ...
    return (
      <Badge className="bg-zinc-100 text-zinc-500 border-zinc-200 hover:bg-zinc-200 rounded-full px-3">
        {current}
      </Badge>
    );
  };
      ...
      <div className="overflow-x-auto w-full pb-4">
        {processedRequests.length === 0 ? (
          <div className="p-16 text-center text-zinc-500 font-medium bg-white border border-black/5 rounded-[2rem] mt-4">
            No requests match your current filters. Try adjusting your search or
            clearing the dropdowns.
          </div>
        ) : (
          <Table className="min-w-[800px]">
            <TableHeader className="bg-zinc-50">
              <TableRow className="border-black/5">
                <TableHead className="text-zinc-500 font-semibold py-4 pl-6">
                  Client Info
                </TableHead>
                ...
                <TableHead className="text-zinc-500 font-semibold hidden md:table-cell">
                  Key Details
                </TableHead>
            ...
            <TableBody>
              {processedRequests.map((req: any) => {
                ...
                const cleanCountryCode = req.countryCode?.replace(
                  /[^0-9]/g,
                  "",
                );
                ...
                return (
                  <TableRow
                    key={req._id}
                    className="border-b border-black/5 hover:bg-zinc-50/50 transition-colors bg-white"
                  >
                    <TableCell className="py-4 pl-6 align-top">
                      ...
                      >
                        <MessageCircle size={12} /> {req.countryCode}{" "}
                        {req.phone}
                      </a>
                    ...
                    <TableCell className="align-top">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        ...
                        {req.condition === "Used" ? (
                          ...
                          <span className="px-1.5 py-0.5 bg-black/5 border border-black/10 rounded text-[10px] text-black font-medium">
                            Brand New
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col gap-1.5 mt-1">
                        ...
                        {req.paymentType === "Full payment" ? (
                            ...
                            <div className="flex flex-col mt-0.5 pt-0.5 border-t border-blue-200/50">
                              <div className="flex justify-between items-center font-medium">
                                ...
                                <span>
                                  $
                                  {(
                                    req.balancePaymentAmount || 0
                                  ).toLocaleString()}
                                </span>
                              </div>
                              <div className="text-[9px] text-blue-500 uppercase font-bold mt-0.5">
                                Due: {req.balancePaymentStage || "TBD"}
                              </div>
                            </div>
                          </div>
                        ) : req.agreedPrice ? (
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-emerald-700">
                            ...
                            {req.agreedPrice.toLocaleString()}
                          </div>
                        ) : (
                          <div className="text-xs text-zinc-400 italic">
                            No pricing terms set
                          </div>
                        )}

                        {/* SHIPPING INFO DISPLAY */}
                        {(req.status === "Shipped" ||
                          req.status === "Arrived at Port" ||
                          req.status === "Cleared Customs") && (
                              ...
                              <div className="flex items-center gap-1.5 text-blue-600">
                                ...
                                {req.eta && (
                                  <span className="text-[10px] text-zinc-400">
                                    (ETA:{" "}
                                    {new Date(req.eta).toLocaleDateString()})
                                  </span>
                                )}
                              </div>
                            )}
                      ...
                      <DropdownMenu>
                        ...
                        >
                          ...
                          <DropdownMenuItem
                            ...
                            className="font-bold text-blue-600 hover:bg-blue-50 focus:bg-blue-50 cursor-pointer rounded-lg mb-1 py-2"
                          >
                            <Activity className="mr-2 h-4 w-4" /> Update Sales
                            Status
                          </DropdownMenuItem>
                          ...
                          {prevStage && (
                            ...
                            >
                              <ArrowLeft className="mr-2 h-4 w-4" /> Revert to:{" "}
                              {prevStage}
                            </DropdownMenuItem>
                          )}
                          ...
                          <DropdownMenuItem
                            ...
                            className="hover:bg-black/5 focus:bg-black/5 cursor-pointer rounded-lg"
                          >
                            <Eye className="mr-2 h-4 w-4 text-zinc-500" /> View
                            Details & Notes
                          </DropdownMenuItem>
