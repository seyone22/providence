...
import { setFollowUpTimer, clearFollowUpTimer, expireFollowUpTimer } from "@/actions/admin-actions";
...
function TimerRing({ ratio }: { ratio: number }) {
    ...
    return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Track */}
            <circle cx={cx} cy={cy} r={r} stroke="#e5e7eb" strokeWidth="2.5" fill="none" />
            {/* Progress arc */}
            <circle
                cx={cx}
                cy={cy}
...
export default function FollowUpTimer({ requestId, followUpAt, followUpSetAt, onExpired }: FollowUpTimerProps) {
    const [open, setOpen] = useState(false);
    const [ratio, setRatio] = useState<number | null>(null);
    const [expired, setExpired] = useState(false);
    const [saving, setSaving] = useState(false);
    const [clearing, setClearing] = useState(false);
    const [customDate, setCustomDate] = useState(formatDateInput(addDays(new Date(), 7)));
        ...
        <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="sm:max-w-sm bg-white border-black/5 rounded-2xl shadow-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-base font-bold text-black">Set Follow-up Reminder</DialogTitle>
                        <p className="text-xs text-zinc-500 mt-0.5">
                            When the timer expires, the sales status will reset to <strong>Action Required</strong> automatically.
                        </p>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                        {/* Presets */}
                        <div>
                            <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">Quick Select</p>
                        ...
                        {/* Custom date */}
                        <div>
                            <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">Or pick a date</p>
                            <input
                                type="date"
                                min={minDate}
                                value={customDate}
                        ...
                        {customDate && (
                            <div className="bg-zinc-50 border border-zinc-100 rounded-lg px-3 py-2 text-xs text-zinc-600">
                                Follow-up due: <strong className="text-black">{new Date(customDate).toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</strong>
                            </div>
                        )}
                        ...
                        {hasTimer && followUpAtDate && (
                            <div className="bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 text-xs text-amber-800 flex items-center justify-between">
                                <span>Active timer → {followUpAtDate.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                                <button
                                    onClick={handleClear}
                                    disabled={clearing}
                    ...
                    <DialogFooter className="gap-2">
                        <Button variant="ghost" onClick={() => setOpen(false)} className="rounded-lg text-sm">
                            Cancel
                        </Button>
                        <Button
                            onClick={handleSave}
