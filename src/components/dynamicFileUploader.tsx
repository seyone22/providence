  ...
  return (
    <div className="space-y-4 border border-black/5 rounded-2xl p-5 bg-zinc-50/50">
      <div className="flex items-center justify-between border-b border-black/5 pb-3">
        <Label className="text-zinc-800 font-bold text-base">
          Stage Documents
        </Label>
        <Button
          type="button"
          ...
          size="sm"
          onClick={addField}
          className="h-8 rounded-lg gap-1.5 text-xs border-black/10"
        >
      ...
      {pendingFiles.length === 0 ? (
        ...
        <div className="space-y-4">
          {pendingFiles.map((pf) => (
            <div
              key={pf.id}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-white p-3 rounded-xl border border-black/5 shadow-sm"
            >
              <Input
                placeholder="Document Name (e.g. Invoice)"
                value={pf.fieldName}
                onChange={(e) =>
                  updateField(pf.id, "fieldName", e.target.value)
                }
              ...
              <Select
                value={pf.fileType}
                onValueChange={(val) => updateField(pf.id, "fileType", val)}
              >
              ...
              <div className="flex-1 w-full">
                <Input
                  ...
                  onChange={(e) =>
                    updateField(pf.id, "file", e.target.files?.[0] || null)
                  }
