...
export const StaffAlertEmail = ({ data, requestId }: StaffAlertEmailProps) => {
  ...
  return (
    <EmailLayout
      preview={`New Lead: ${data.make} ${data.vehicle_model} for ${data.name}`}
      heading="New Vehicle Request"
    >
      ...
      <Section style={containerStyle}>
        ...
        {Array.isArray(data.contactMethods) &&
          data.contactMethods.length > 0 && (
            <>
              <div
                style={{ margin: "20px 0", borderBottom: "1px solid #e4e4e7" }}
              />
              <Text style={labelStyle}>Preferred Contact</Text>
              <Text style={valueStyle}>
                Method: {data.contactMethods.join(", ")}
              </Text>
              <Text style={valueStyle}>
                When: {data.contactTimeWindow}
                {Array.isArray(data.contactDays) && data.contactDays.length
                  ? ` · ${data.contactDays.join(", ")}`
                  : ""}
              </Text>
              ...
              {data.contactTimezoneLabel && (
                <Text
                  style={{ ...valueStyle, fontSize: "12px", color: "#71717a" }}
                >
