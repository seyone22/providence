...
export const LeadQualifiedAlertEmail = ({
  data,
  requestId,
}: LeadQualifiedEmailProps) => {
  const adminUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/admin`;

  return (
    <EmailLayout
      preview={`Lead Qualified: ${data.name} is interested in ${data.make} ${data.vehicle_model}`}
      heading="Lead Qualified ✅"
    >
      <Text style={textStyle}>
        Great news! A client has actively engaged with their assigned agent (via
        Email or WhatsApp) directly from the tracking dashboard. Their lead
        status has automatically been updated to <strong>Qualified</strong>.
      </Text>
