// Adds the grade / steering columns to one environment's database.
//
// These are the columns from drizzle/0004_grade_columns.sql. They're applied
...
//
//   node --env-file=.env.local scripts/apply-grade-columns.mjs dev
//   node --env-file=.env.local scripts/apply-grade-columns.mjs dev --apply
//
// Default is a read-only check. Nothing is written without --apply.
//
// Every statement is ADD COLUMN IF NOT EXISTS, and the two dossier columns
// default to empty, so applying twice is a no-op and applying ahead of the
// deploy is harmless: dossiers read as "no ladder, one hand" until an admin
// fills one in, and leads as "grade not asked".
...
const COLUMNS = [
  ["specdossier", "grades"],
  ["specdossier", "steeringOptions"],
  ["request", "grade"],
  ["request", "steering"],
];

const STATEMENTS = [
  `ALTER TABLE "specdossier" ADD COLUMN IF NOT EXISTS "grades" jsonb DEFAULT '[]'::jsonb NOT NULL`,
  `ALTER TABLE "specdossier" ADD COLUMN IF NOT EXISTS "steeringOptions" text[] DEFAULT '{}'::text[] NOT NULL`,
  `ALTER TABLE "request" ADD COLUMN IF NOT EXISTS "grade" text`,
...
async function run() {
  ...
  if (!envName || !ENVS[envName]) {
    fail(
      `Usage: node --env-file=.env.local scripts/apply-grade-columns.mjs <${Object.keys(
        ENVS,
      ).join("|")}> [--apply]`,
    );
  }
  ...
  try {
    ...
    // All four or none: a dossier table that has `grades` but a request table
    // that has no `grade` column would take inquiries and drop the answer.
